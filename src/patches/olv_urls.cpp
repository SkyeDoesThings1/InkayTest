...
void new_rpl_loaded(OSDynLoad_Module module, void* ctx, OSDynLoad_NotifyReason reason, OSDynLoad_NotifyData* rpl) {
    if (!Config::connect_to_network) {
        DEBUG_FUNCTION_LINE("Inkay-Miiverse: Miiverse patches skipped.");
        return;
    }
...
bool setup_olv_libs() {
    ...
    if (!olvLoaded) {
        DEBUG_FUNCTION_LINE("Inkay-Miiverse: no olv, quitting for now\n");
        return false;
    }
    ...
    if (OSGetMemBound(OS_MEM2, &base_addr, &size)) {
        DEBUG_FUNCTION_LINE("Inkay-Miiverse: OSGetMemBound failed!");
        return false;
    }
